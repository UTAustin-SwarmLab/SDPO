...
# Usage: USE_FUTURE_RETURNS=True USE_FUTURE_RETURNS_BASELINE=True FUTURE_RETURNS_BASELINE_MODE=group FUTURE_RETURNS_REWARD_SCALE=none GAMMA=1.0 ENABLE_ICL=True ./run_sdpo_nofulllogit_all.sh [--dry-run]
#
...
# Set USE_FUTURE_RETURNS=True to enable discounted future returns (default: False).
...
# Set FUTURE_RETURNS_BASELINE_MODE=group|batch (default: group).
# Sampled-token baseline is G(r - E[r_t]). Set FUTURE_RETURNS_REWARD_SCALE=none|remaining|seq_len
# (default: none) to divide r_t before that baseline/cumsum; appends -s* to the gae tag.
...
# is_clip enables loss-level IS (value unused for clipping on the sampled-token path).
IS_CLIPS=(2.0)
# CISPO-style symmetric clip for sampled-token path: ratio ∈ [1-cispo_clip, 1+cispo_clip].
CISPO_CLIPS=(0.2)
MODEL_PATHS=(
                            ...
                            for IS_CLIP in "${IS_CLIPS[@]}"; do
                                for CISPO_CLIP in "${CISPO_CLIPS[@]}"; do
                                    for DATA_PATH in "${DATA_PATHS[@]}"; do
                                        EXP_NAME="FINAL-SDPO-nofulllogit-${ICL_TAG}-${RETURNS_TAG}-${CLAMP_TAG}-isclip${IS_CLIP}-cispoclip${CISPO_CLIP}-mbs-${MINI_BATCH_SIZE}-train${TRAIN_BATCH_SIZE}-rollout${ROLLOUT_BATCH_SIZE}-lr${LR}-alpha${ALPHA}-model${MODEL_PATH}"
                                        CMD=(sbatch -A ASC26054 "$TACC_DIR/jobs/run_sdpo_nofulllogit.slurm" "${DATA_PATH}" "${TRAIN_BATCH_SIZE}" "${ROLLOUT_BATCH_SIZE}" "${MINI_BATCH_SIZE}" "${LR}" "${MODEL_PATH}" "${ALPHA}" "${DONTS_REPROMPT_ON_SELF_SUCCESS}" "${EXP_NAME}" "${TOPK}" "${TEACHER_UPDATE_RATE}" "${ENABLE_ICL}" "${CLAMP_HIGH}" "${CLAMP_LOW}" "${USE_REWARD_CLAMP}" "${IS_CLIP}" "${USE_FUTURE_RETURNS}" "${GAMMA}" "${USE_FUTURE_RETURNS_BASELINE}" "${CISPO_CLIP}" "${FUTURE_RETURNS_REWARD_SCALE}" "${FUTURE_RETURNS_BASELINE_MODE}")
