...
def compute_distil_self_distillation_loss(
    ...
    """DistIL: forward CE + future-credit policy-gradient loss."""
    metrics = {}
    ...
    if self_distillation_config.full_logit_distillation:
        ...
        teacher_distill_probs = torch.exp(teacher_distill_log_probs)
        ce_per_token = -(teacher_distill_probs * student_distill_log_probs).sum(-1)
        ce_per_token = torch.clamp(ce_per_token, min=0.0)

        with torch.no_grad():
            token_rewards = -(teacher_distill_probs * student_distill_log_probs).sum(-1)
            token_rewards = torch.clamp(token_rewards, min=0.0)
            shifted = torch.cat([token_rewards[:, 1:], torch.zeros_like(token_rewards[:, :1])], dim=1)
            future_returns = torch.flip(
                torch.cumsum(torch.flip(shifted, dims=[1]), dim=1), dims=[1]
            )
            seq_len = loss_mask.shape[1]
            positions = torch.arange(seq_len, device=loss_mask.device).unsqueeze(0)
            total_len = loss_mask.sum(dim=1, keepdim=True).clamp(min=1.0)
            remaining = (total_len - positions - 1).clamp(min=1.0)
            future_returns = future_returns / remaining

        pg_per_token = student_log_probs * future_returns

        with torch.no_grad():
            num_valid = loss_mask.sum().clamp(min=1.0)
            metrics["distil/ce_loss"] = ((ce_per_token * loss_mask).sum() / num_valid).item()
            metrics["distil/pg_loss"] = ((pg_per_token * loss_mask).sum() / num_valid).item()
            metrics["distil/returns_mean"] = ((future_returns * loss_mask).sum() / num_valid).item()
            metrics["distil/returns_max"] = (future_returns * loss_mask).max().item()
